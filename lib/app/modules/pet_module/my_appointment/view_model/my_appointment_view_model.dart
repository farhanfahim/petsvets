...
class MyAppointmentViewModel extends GetxController {
  ...
  Timer? debounce;
  MyAppointmentViewModel({required this.repository});
  ...
  Future<dynamic>  getVets({bool isRefresh = false,int type = 0}) async {
    ...
    var map = {
      'page': currentPage,
      'relations':['user','vet'],
      if(searchController.value.text.isNotEmpty)'search':searchController.value.text,
      if(type != 0)'status[]': type,
    };
